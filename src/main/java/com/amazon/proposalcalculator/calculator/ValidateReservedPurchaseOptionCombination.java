...
public class ValidateReservedPurchaseOptionCombination {
	...
	public static void validate(InstanceInput input) {
		...
		combinations.add(
				new PossiblePurchaseCombination(TermType.Reserved.name(), LeaseContractLength.ONE_YEAR.getColumnName(),
		...
		combinations.add(new PossiblePurchaseCombination(TermType.Reserved.name(),
				LeaseContractLength.THREE_YEARS.getColumnName(), PurchaseOption.NO_UPFRONT.getColumnName(),
				OfferingClass.Standard.name()));
		combinations.add(new PossiblePurchaseCombination(TermType.Reserved.name(),
				LeaseContractLength.THREE_YEARS.getColumnName(), PurchaseOption.PARTIAL_UPFRONT.getColumnName(),
				OfferingClass.Standard.name()));
		combinations.add(new PossiblePurchaseCombination(TermType.Reserved.name(),
				LeaseContractLength.THREE_YEARS.getColumnName(), PurchaseOption.ALL_UPFRONT.getColumnName(),
				OfferingClass.Standard.name()));
		combinations.add(new PossiblePurchaseCombination(TermType.Reserved.name(),
				LeaseContractLength.THREE_YEARS.getColumnName(), PurchaseOption.NO_UPFRONT.getColumnName(),
		...
		List<PossiblePurchaseCombination> validCombinations = combinations.stream()
				.filter(getValidCombination(new PossiblePurchaseCombination(input.getTermType(),
						input.getLeaseContractLength(), input.getPurchaseOption(), input.getOfferingClass())))
				.collect(Collectors.toList());

		if (validCombinations.isEmpty()) {
			throw new PricingCalculatorException(INVALID_COMBINATION + " Combination found:" + input.getTermType()
					+ ", " + input.getLeaseContractLength() + ", " + input.getPurchaseOption() + ", "
	...
	private static Predicate<PossiblePurchaseCombination> getValidCombination(PossiblePurchaseCombination combination) {
		return p -> p.termType.equals(TermType.getTermType(combination.termType).name())
				&& ((p.leaseContractLength == null && combination.leaseContractLength == null)
						|| (p.leaseContractLength != null && p.leaseContractLength.equals(LeaseContractLength
								.getLeaseContractLength(combination.leaseContractLength).getColumnName())))
				&& ((p.offeringClass == null && combination.offeringClass == null) || (p.offeringClass != null
						&& p.offeringClass.equals(OfferingClass.getOfferingClass(combination.offeringClass).name())))
				&& ((p.purchaseOption == null && combination.offeringClass == null)
						|| (p.purchaseOption != null && p.purchaseOption
								.equals(PurchaseOption.getPurchaseOption(combination.purchaseOption).getColumnName())));
	}
	...
	private static class PossiblePurchaseCombination {
		...
		public PossiblePurchaseCombination(String termType, String leaseContractLength, String purchaseOption,
